...
class Command(BaseCommand):
    help = 'Create sample bonuses, gifts, and schemes for testing'
    ...
    def handle(self, *args, **options):
        ...
        for gift_data in gifts_data:
            ...
            gifts.append(gift)
            if created:
                self.stdout.write(self.style.SUCCESS(f'Created gift: {gift}'))
        ...
        try:
            ...
            if medicines:
                ...
                for bonus_datum in bonus_data:
                    ...
                    if created:
                        self.stdout.write(self.style.SUCCESS(f'Created bonus: {bonus}'))
            else:
                self.stdout.write(self.style.WARNING('No medicines found. Please add some medicines first.'))
        
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'Error creating bonuses: {e}'))
        ...
        if schemes:
            ...
            self.stdout.write(self.style.SUCCESS(f'Associated {len(gifts)} gifts with {schemes[0].name}'))
            
            schemes[1].gifts.set(gifts[:3])
            self.stdout.write(self.style.SUCCESS(f'Associated {len(gifts[:3])} gifts with {schemes[1].name}'))
            
            schemes[2].gifts.set(gifts)
            self.stdout.write(self.style.SUCCESS(f'Associated {len(gifts)} gifts with {schemes[2].name}'))
